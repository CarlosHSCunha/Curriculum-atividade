...
const projetos = [
  {
    ...
    description: "Site de portfólio desenvolvido com Next.js, React e Tailwind CSS. Apresenta minha trajetória acadêmica, experiência profissional e projetos.",
  ...
  return (
    <section className="mx-auto max-w-5xl px-6 pt-28 sm:pt-32 pb-20">
      <header className="mb-12">
        ...
        <p className="mt-3 text-muted-foreground">Coisas que construí ao longo da minha jornada.</p>
      </header>

      <ul className="grid sm:grid-cols-2 gap-5">
        {projetos.map((p) => (
          <li key={p.title} className="rounded-xl border border-border bg-card p-6 flex flex-col justify-between hover:border-primary/60 transition-colors">
            <div>
              <h2 className="text-lg font-semibold text-foreground">{p.title}</h2>
              <p className="mt-2 text-sm text-muted-foreground leading-relaxed">{p.description}</p>
              <div className="mt-4 flex flex-wrap gap-2">
                {p.techs.map((t) => (
                  <span key={t} className="text-xs px-3 py-1 rounded-full border border-border bg-secondary/40 text-foreground">
                    {t}
                  </span>
                ))}
              </div>
